...
% - 2020-10-29 19:03:02 - for some reasons strings are read as cell
% strings. Adding convertoin back to char array. RC
% - 2021-05-19 20:46:17 - empty name of the dataset debugged  RC
...
for ii=1:length(datasets)
    ...
    if isempty(name) % - 2021-05-19 20:46:17 -   RC
        continue;
    end
    if strcmp(name,options.skip)
        ...
        metadata.(name)=h5read(fpath,['/', name]); % using h5 read for datasets as h5laod returns errors % - 2020-06-30 00:03:41 -   RC
            ...
            warning('Cannot load %s dataset',name);
            continue;
        end
        if iscellstr(metadata.(name)) % - 2020-10-29 19:03:02 -   RC
